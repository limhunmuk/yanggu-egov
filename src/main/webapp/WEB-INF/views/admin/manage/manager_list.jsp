...
<div id="wrap" class="skin_type01">
	...
	<div class="sub_wrap">
		...
		<div class="container clearfix">
			<div class="content">
				<div class="list">
					<form action="/admin/manage/managerList">
						<table class="search">
							<caption>검색</caption>
							<colgroup>
								<col style="width:20%;">
					...
					<div class="search_wrap">
								...
								<c:otherwise>
									<c:forEach var="item" items="${list}" varStatus="status">
										<tr>
											...
											<td><a href="addManager?seq=${item.seq}">${item.adminId}</a></td>
											<td><a href="addManager?seq=${item.seq}">${item.adminName}</a></td>
											<td><a href="addManager?seq=${item.seq}">${item.adminEmail}</a></td>
											<td><a href="addManager?seq=${item.seq}">${item.adminMobile}</a></td>
											<td><a href="addManager?seq=${item.seq}">${item.adminDept}</a></td>
											...
											<td><a href="addManager?seq=${item.seq}">${fn:substring(auth,1,fn:length(auth)) }</a></td>
										</tr>
									</c:forEach>
								</c:otherwise>
							</c:choose>
						...
						<div class="align_r mt20">
							<button type="button" class="btn_search" onclick="location.href = '/admin/manage/addManager'">등록</button>
						</div>
						<!-- 페이징 -->
...
<form action="/admin/manage/managerList/excel" method="post" id="excel">
