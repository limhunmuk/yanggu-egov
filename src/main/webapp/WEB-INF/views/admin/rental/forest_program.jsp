								...
								<tr>
									...
									<td>
										<select style="width:200px;" name="gubun" id="gubun">
											...
											<option value="e" <c:if test="${gubun eq 'e'}">selected="selected"</c:if>>유아숲체험</option>
											<option value="f" <c:if test="${gubun eq 'f'}">selected="selected"</c:if>>숲 해설</option>
										</select>
									</td>
								</tr>
								<tr>
...
$(function(){
	...
	$('#lnb ul.menu > li').eq(3).find(' > a ').addClass('skin_bg').next().show().find('>li').eq(${subMenuNo}).addClass('on');
...
function getOpenDate(gubun){
	...
	$.ajax({
	    ...
	    , success : function (data) {
	    	...
	    	var str="";
	    		str+= '현재 오픈일 : '+data.result['startDate']+'시'+ ' ~ ' + data.result['endDate']+'시';
...
function add_openDateInsert(){	
	...
	$.post( "/admin/forest/openDate", $("#frm").serialize() , function(data) {
