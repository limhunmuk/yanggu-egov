...
<script src="/ad_js/cust/forest_rental.js?v=20230914-1"></script>
...
// 	if ($(obj).val() == 'b') {
		...
		if(flag){
			$.ajax({
				url : "/admin/rental/forestStatUpdate"
...
// 	}
	...
	function search(){
		var gubun = '${gubun}'
		var subMenuNo = '${subMenuNo}'
		$("#searchFrm").attr("action","/admin/rental/forestRentalList?gubun="+gubun+"&subMenuNo="+subMenuNo);
		$("#searchFrm").submit();
	}
	...
	function excelDownload(){
		...
		$("#searchFrm").attr("action","/admin/rental/forestRentalList/excel");
		$("#searchFrm").submit();
	}
	...
	function insert(){
		...
		if(confirm("예약 하시겠습니까?")){
			...
			$.post( "/program/checkDayCloseForest",{"useDate":useDate,"gubun":gubun}, function(result) {
				console.log("예약 po_ret : " + result.result.po_ret);
				console.log("예약 po_aCnt : " + result.result.po_aCnt);
				console.log("예약 po_bCnt : " + result.result.po_bCnt);

				var po_ret = result.result.po_ret;
				var po_aCnt = 0;
				var po_bCnt = 0;
				var po_cCnt = 0;
				if (entryTime == 1) {
					po_aCnt = result.result.po_aCnt;
				}else if(entryTime == 2){
					po_bCnt = result.result.po_bCnt;
				}else{
					po_cCnt = result.result.po_cCnt;
				}
				var closeYn = result.result.po_closeYn;
				if(po_ret==210){
					if(po_aCnt>=cnt || po_bCnt>=cnt || po_cCnt>=cnt){
						$.post( "/admin/rental/insertForestExperienceAdmin",$("#insertFrm").serialize(), function(result) {
