    ...
    <div class="sub_wrap">
		...
		<div class="container clearfix">
			<div class="content">
				...
				<div class="list">
					<form action="/admin/site/noticeList" method="post">
						<table class="search">
							<caption>검색</caption>
							<colgroup>
								<col style="width:14%;">
						...
						<div class="align_r mt20">
							<button type="button" class="btn_down" onclick="location.href = 'noticeWrite';">등록</button>
								...
								<c:choose>
									...
									<c:otherwise>
										<c:forEach var="item" items="${list}" varStatus="status">
											<tr>
												<c:if test="${item.kind == 'Y' }"><td><span class="c_red">공지</span></td></c:if>
												<c:if test="${item.kind == 'N' }"><td>${totalCount - ((page -1) * 10 + status.index)}</td></c:if>
												<td class="t_left"><a href="noticeWrite?seq=${item.seq }">${item.title }</a></td>
												<td>${item.adminName }</td>
												<td><c:if test="${item.attachment1 != '' }"><a href="javascript:void(0);" onclick="fileDownLoad('notice','${item.attachment1}','${item.attachment1_org}');"><i class="ico_file">첨부파일</i></a></c:if></td>
												<td><c:if test="${not empty item.attachment2 }"><a href="javascript:void(0);" onclick="fileDownLoad('notice','${item.attachment2}','${item.attachment2_org}');"><i class="ico_file">첨부파일</i></a></c:if></td>
												<td><c:if test="${not empty item.attachment3 }"><a href="javascript:void(0);" onclick="fileDownLoad('notice','${item.attachment3}','${item.attachment3_org}');"><i class="ico_file">첨부파일</i></a></c:if></td>
