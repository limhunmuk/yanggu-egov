				...
				<c:choose>
					...
					<c:otherwise>
						<c:forEach var="item" items="${list}" varStatus="status">	
							<li>
								<div class="title">
									<a href="notice_view?seq=<c:out value="${item.seq }"/>" class=""><c:if test="${item.kind == 'Y' }"><em class="bg_green">공지</em></c:if><c:out value="${item.title }"/></a>
									<i class="new_i"></i>
								</div>
								...
								<c:if test="${not empty item.attachment1 or not empty item.attachment2 or not empty item.attachment3}">
									<div class="down_box">
										<c:if test="${not empty item.attachment1 }"><a href="javascript:fileDownLoad('notice','${item.attachment1}','${item.attachment1_org}');" class="down_btn"><i></i><span>${item.attachment1_org}</span></a></c:if>
										<c:if test="${not empty item.attachment2 }"><a href="javascript:fileDownLoad('notice','${item.attachment2}','${item.attachment2_org}');" class="down_btn"><i></i><span>${item.attachment2_org}</span></a></c:if>
										<c:if test="${not empty item.attachment3 }"><a href="javascript:fileDownLoad('notice','${item.attachment3}','${item.attachment3_org}');" class="down_btn"><i></i><span>${item.attachment3_org}</span></a></c:if>
									</div>
								</c:if>
							</li>
						</c:forEach>
