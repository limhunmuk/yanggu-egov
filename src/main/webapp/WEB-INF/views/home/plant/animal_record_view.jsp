			...
			<div id="container" class="inner  ">
				<div class="view_container">
					...
					<c:if test="${not empty one.attachment1 or not empty one.attachment2 or not empty one.attachment3}">
						...
						<ul class="file_list">
							<li><c:if test="${not empty one.attachment1 }"><a href="javascript:fileDownLoad('ecology','${one.attachment1}','${one.attachment1_org}');" class="down_btn"><i></i><span>${one.attachment1_org}</span></a></c:if></li>
							<li><c:if test="${not empty one.attachment2 }"><a href="javascript:fileDownLoad('ecology','${one.attachment2}','${one.attachment2_org}');" class="down_btn"><i></i><span>${one.attachment2_org}</span></a></c:if></li>
							<li><c:if test="${not empty one.attachment3 }"><a href="javascript:fileDownLoad('ecology','${one.attachment3}','${one.attachment3_org}');" class="down_btn"><i></i><span>${one.attachment3_org}</span></a></c:if></li>
						</ul>
					</c:if>
                    <div class="btn_area">
                        <a href="animal_record_list" class="btn list_btn"><i></i>목록</a>
                    </div>
