...
(function (factory) {
  ...
  var clndrTemplate =
    "<div class='clndr-controls'>" +
        "<div class='clndr-control-button'>" +
            "<span class='clndr-previous-button'>이전달</span>" +
        '</div>' +
        "<div class='month' data-nowMonth='<%= month %>' data-nowYear='<%= year %>'><%= year %>.<% if(month<10){ %>0<% } %><%= month %></div>" +
        "<div class='clndr-control-button rightalign'>" +
            "<span class='clndr-next-button'>다음달</span>" +
        '</div>' +
    '</div>' +
    "<table class='clndr-table' border='0' cellspacing='0' cellpadding='0'>" +
        ...
        '<% for(var i = 0; i < numberOfRows; i++){ %>' +
            ...
            '<% for(var j = 0; j < 7; j++){ %>' +
            '<% var d = j + i * 7; %>' +
                "<td class='<%= days[d].classes %>' id='td-<%= days[d].id %>' data-day='<%= days[d].id %>' data-week='<%= j %>' data-old='0'>" +
                    "<em class='day-contents'><%= days[d].day %></em>" +
	  				"<div id='reserve_<%= days[d].id %>' data-close='0' class='box'></div>" +
                '</td>' +
            '<% } %>' +
            '</tr>' +
        '<% } %>' +
  ...
  Clndr.prototype.init = function () {
    ...
    if (!this.options.daysOfTheWeek) {
      this.daysOfTheWeek = [];
	  this.daysOfTheWeek.push("일");
	  this.daysOfTheWeek.push("월");
	  this.daysOfTheWeek.push("화");
	  this.daysOfTheWeek.push("수");
	  this.daysOfTheWeek.push("목");
	  this.daysOfTheWeek.push("금");
	  this.daysOfTheWeek.push("토");
/*영어 요일 첫자
      for (i = 0; i < 7; i++) {
...
*/
  ...
  Clndr.prototype.createDayObject = function (day, monthEvents) {
	...
	var idElement = day.format('YYYY-MM-DD');
  ...
  Clndr.prototype.render = function () {
    ...
    if (this.options.lengthOfTime.days) {
      ...
      data = {
        ...
        extras: this.options.extras,
        //month: this.month.format('MMMM'), //ㅈJune 같이 영어식 표기
