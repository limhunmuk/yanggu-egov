...
//    console.log(myweekday);
	...
	} else if (sw==8) { // yyyymmdd
		ret = ( mydate.getFullYear()+((mymonth < 10) ? "0" : "") + mymonth + ((myweekday < 10) ? "0" : "") + myweekday );
	} else if (sw==9) { // yyyy.mm.dd
		ret = ( mydate.getFullYear()+"." + ((mymonth < 10) ? "0" : "")+ mymonth + "." + ((myweekday < 10) ? "0" : "") + myweekday );
	} else if (sw==10) { // yyyy년 mm월 dd일
		ret = ( mydate.getFullYear()+"년 " + ((mymonth < 10) ? "0" : "") + mymonth + "월 " + ((myweekday < 10) ? "0" : "") + myweekday + "일" );
	}
    return ret;
}
//날짜 포맷
...
function PageSet(totalCnt,dataSize,pageSize,pageNo){
	...
	if(pageNo>pageSize){
		...
		html.push('<a title="이전 페이지로" class="prev" href="javascript:go_paging(');
		html.push(1);
		html.push(')";>');
		html.push('이전페이지</a>');
	}
	for(var index=pRCnt*pageSize+1;index<(pRCnt+1)*pageSize+1;index++){
		if(index==pageNo){
			html.push('<a title="현재 '+index+'페이지" class="on" href="javascript:go_paging(');
			html.push(index);
			html.push(')";>');
			html.push(index);
			html.push('</a>');
		}else{
			html.push('<a title="'+index+'페이지 이동" href="javascript:go_paging(');
			html.push(index);
			html.push(')";>');
			html.push(index);
			html.push('</a>');
	...
	if(pageCnt>(pRCnt+1)*pageSize){
		html.push('<a title="다음 페이지로" class="next" href="javascript:go_paging(');
...
var isEmpty = function(value){ 		//null체크 empty값이면 true반환
...
function doNotReload(){
	...
	for (number in allowPageList) { 
		...
		if (regExp.test(document.location.pathname)) { bBlockF5Key = false; break; } 
		} 
	if (bBlockF5Key) { 
		if (e.which === 116) { 
