...
function dayReserveState(cseq,mydate) {			
	$.post('/API', {kd:'C',cseq:cseq,ctype:'F',date:mydate} , function(result) {
		...
		for (var i = 0; i < result['list'].length; i++) {
			...
			if(stat=='O'){
				...
				if(_k==0){
					_darkAdultFee = String(nullCheck(result['list'][0].darkAdultFee, ''));
					console.log("금액"+_darkAdultFee);
					_darkMixFee = String(nullCheck(result['list'][0].darkMixFee, ''));
					_darkChildFee = String(nullCheck(result['list'][0].darkChildFee, ''));
					
					_adventureAdultFee = String(nullCheck(result['list'][0].adventureAdultFee, ''));
					_adventureMixFee = String(nullCheck(result['list'][0].adventureMixFee, ''));
					_adventureChildFee = String(nullCheck(result['list'][0].adventureChildFee, ''));
					
					_cooperationAdultFee = String(nullCheck(result['list'][0].cooperationAdultFee, ''));
					_cooperationMixFee = String(nullCheck(result['list'][0].cooperationMixFee, ''));
					_cooperationChildFee = String(nullCheck(result['list'][0].cooperationChildFee, ''));
					_k=1;
				}
